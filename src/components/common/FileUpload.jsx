...
export default function FileUpload({
  label, required, maxSizeMB = MAX_SIZE_MB, onFileAccepted, existingFile,
}) {
    ...
    <div className="field">
      ...
      <div {...getRootProps()} className={`border-2 border-dashed rounded-md p-4 cursor-pointer text-center ${isDragActive ? 'border-primary bg-primary/5' : 'border-gray-300'} ${status === 'error' ? 'border-danger' : ''} ${status === 'done' ? 'border-accent bg-accent/5' : ''}`}>
        ...
        <p className="text-sm text-gray-500">
          Drag a file here, or tap to choose one. PDF, JPG, PNG — up to
          {maxSizeMB}
          MB.
        </p>
        )}
        ...
        <div className="flex items-center gap-3 justify-center">
          ...
          <div className="text-left text-sm">
            <p className="text-accent font-medium">Uploaded</p>
            {sizeInfo?.compressed && (
