...
export default function Step3KYC({
  ...
  const {
    ...
    defaultValues: {
      pan: formState.pan, aadhaar: formState.aadhaar, aadhaarConsent: formState.aadhaarConsent, voterId: formState.voterId, passport: formState.passport,
    },
  });
  const pan = watch('pan'); const
    aadhaar = watch('aadhaar');
  const panVerify = useVerification('pan', formState.loanType); const
    aadhaarVerify = useVerification('aadhaar');
