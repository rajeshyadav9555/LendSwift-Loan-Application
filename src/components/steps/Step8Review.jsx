...
export default function Step8Review({formState,onNext,onBack,isFirstStep,isLastStep}) {
  ...
  return <form onSubmit={(e)=>{e.preventDefault();onNext({});}}>
    ...
    <div className="border rounded-md divide-y">
      {rows.map(([label,value])=><div key={label} className="flex justify-between gap-4 px-4 py-3 text-sm"><span className="text-gray-500">{label}</span><span className="font-medium text-right">{value}</span></div>)}
    </div>
    <div className="mt-4 p-4 rounded-md bg-gray-50 text-sm text-gray-600">
      Interest rate: {summary.annualRate}% p.a. · Total payable: {formatINRCurrency(summary.totalPayable)}
    </div>
