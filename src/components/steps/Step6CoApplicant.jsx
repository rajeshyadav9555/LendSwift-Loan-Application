...
export default function Step6CoApplicant({formState,onNext,onBack,isFirstStep,isLastStep}) {
  const required=isCoApplicantStepRequired(formState.loanType,formState.loanAmount);
  const {register,control,handleSubmit,formState:{errors}}=useForm({
    resolver:zodResolver(step6Schema),
    defaultValues:{coApplicantRequired:required||formState.coApplicantRequired,coApplicantName:formState.coApplicantName,relationship:formState.relationship,coApplicantPan:formState.coApplicantPan,coApplicantIncome:formState.coApplicantIncome,coApplicantConsent:formState.coApplicantConsent}
  });
