...
export const step4Schema = z.object({
  ...
  if (data.residenceType === 'rented' && (!data.rentAmount || data.rentAmount <= 0)) ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['rentAmount'], message: 'Enter your monthly rent amount.' });
  if (!data.sameAsPermanent) {
    if (!data.permanentAddressLine1 || data.permanentAddressLine1.length < 5) ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['permanentAddressLine1'], message: 'Enter your permanent address.' });
    if (!data.permanentPincode || !/^\d{6}$/.test(data.permanentPincode)) ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['permanentPincode'], message: 'Enter a valid 6-digit permanent address PIN code.' });
  }
});
